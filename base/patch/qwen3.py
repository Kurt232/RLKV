...
from transformers.models.qwen3.modeling_qwen3 import Qwen3ForCausalLM, Qwen3Model, apply_rotary_pos_emb
...
from base.tuple_kv_cache import enable_tuple_kv_cache_for_qwen3
...
def qwen3_duo_attention_forward_two_way(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        full_query_states = self.q_norm(full_query_states.view(
            bsz, q_len, self.config.num_attention_heads, self.head_dim
        ))
        full_key_states = self.k_norm(full_key_states.view(
    ...
    streaming_query_states = self.q_norm(streaming_query_states.view(
        bsz, q_len, self.config.num_attention_heads, self.head_dim
    ))
    streaming_key_states = self.k_norm(streaming_key_states.view(
        bsz, q_len, self.config.num_key_value_heads, self.head_dim
    ))
    ...
    with torch.no_grad():
        full_attn_output = full_attn_output.reshape(bsz, q_len, -1)
...
def qwen3_duo_attention_forward_one_way_reordered(
    self,
    hidden_states: torch.Tensor,
    position_embeddings: Tuple[torch.Tensor, torch.Tensor],
...
def enable_qwen3_duo_attention_eval(
    model: Qwen3ForCausalLM,
    full_attention_heads,
    sink_size,
    recent_size,
    ...
    for idx, layer in enumerate(model.model.layers):
        ...
        module.forward = types.MethodType(
            qwen3_duo_attention_forward_one_way_reordered, module
        )
