...
def old_flash_attention_2_forward_qwen3(
    self,
    hidden_states: torch.Tensor,
    position_embeddings: Tuple[torch.Tensor, torch.Tensor],
    attention_mask: Optional[torch.Tensor],
...
def _upad_input(self, query_layer, key_layer, value_layer, padding_mask, query_length):
    ...
    if query_length == kv_seq_len:
        query_layer = index_first_axis(
            query_layer.reshape(
                batch_size * kv_seq_len, self.config.num_attention_heads, head_dim
            ),
            indices_k,
        )
...
def enable_tuple_kv_cache_for_qwen3(model: Qwen3ForCausalLM):
    ...
    for idx in range(len(model.model.layers)):
        ...
        model.model.layers[idx].self_attn.forward = types.MethodType(
            old_flash_attention_2_forward_qwen3, model.model.layers[idx].self_attn
        )
        model.model.layers[idx].self_attn._upad_input = types.MethodType(
            _upad_input, model.model.layers[idx].self_attn
        )
