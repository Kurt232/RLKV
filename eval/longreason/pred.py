...
def parse_args(args=None):
    ...
    parser.add_argument("--task", type=str, help="task name", default="longreason")
...
def get_pred(
    ...
    for idx, json_obj in enumerate(pbar):
        question = json_obj["final_inquiry"]
        ...
        example_idx = json_obj["example_idx"]
...
def load_model_and_tokenizer(args, path):
    ...
    elif args.method == "rlkv" or args.method == "kvzip":
        assert args.attn_load_dir is not None, "attn_load_dir must be provided"
        print(
            f"Loading attention pattern from {args.attn_load_dir} with sparsity {args.sparsity}"
        )
...
if __name__ == "__main__":
    ...
    root = "eval/longreason/pred_200"
    if not os.path.exists(root):
        os.makedirs(root)

    data = load_dataset("lz1bytedance/LongReason", split="64k")
    size_ = len(data)
    if size_ > 200:
        data = data.shuffle(seed=42).take(200)
    ...
    elif args.method == "kvzip":
        out_path = f"{root}/{model_name}/{dataset}-kvzip-{args.attn_load_dir.split('/')[-1]}-sp-{args.sparsity}.jsonl"
